...
\
  ...
  { \
    TENSOR##_data = THTensor_getStoragePtr(TENSOR)->template data<TYPE>()+TENSOR->storage_offset(); \
    TENSOR##_size = 1; \
    TENSOR##_stride = 1; \
...
{\
  ...
  if(TENSOR##Contg){                                                        \
    ptrdiff_t iter = 0;                                                     \
    TYPE *rp = THTensor_getStoragePtr(TENSOR)->template data<TYPE>()+TENSOR->storage_offset();         \
...
{                                                                                              \
  ...
  if( CONTIG1 && CONTIG2 ){                                                                    \
    TYPE1 *rp = THTensor_getStoragePtr(TENSOR1)->template data<TYPE1>()+TENSOR1->storage_offset();                        \
    TYPE2 *tp = THTensor_getStoragePtr(TENSOR2)->template data<TYPE2>()+TENSOR2->storage_offset();                        \
    ptrdiff_t iter = 0;                                                                        \
    if(tp != (TYPE2*)rp) {                                                                             \
      PRAGMA(ivdep) \
...
{                                                                             \
  ...
  if(CONTIG1 && CONTIG2 && CONTIG3){                                                                    \
    TYPE1 *rp = THTensor_getStoragePtr(TENSOR1)->template data<TYPE1>()+TENSOR1->storage_offset();                                 \
    TYPE2 *tp = THTensor_getStoragePtr(TENSOR2)->template data<TYPE2>()+TENSOR2->storage_offset();                                 \
    TYPE3 *srcp = THTensor_getStoragePtr(TENSOR3)->template data<TYPE3>()+TENSOR3->storage_offset();                               \
    ptrdiff_t iter = 0;\
    if(tp != (TYPE2*)rp) {                                                                             \
      PRAGMA(ivdep) \
